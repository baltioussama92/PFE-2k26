...
export default function UserDetails() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([foundUser, historyRows, chatRows, listingRows, bookingRows, earningRows]) => {
        if (!active) return
        if (!foundUser) {
  ...
  const onSaveProfile = async () => {
    ...
    setUser(updated)
    showToast('Profile updated on frontend.')
  }
  ...
  const onChangePassword = async () => {
    ...
    await adminApi.changeUserPasswordFrontendOnly()
    setSavingPassword(false)
    setPasswordForm({ next: '', confirm: '' })
    showToast('Password changed on frontend.')
  }
  ...
  const onDeleteUser = async () => {
    ...
    setDeleting(false)
    showToast('Account deleted on frontend.')
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'chats' && (
        <Card title="Chats" subtitle="Conversation records visible to the current admin account">
          {chats.length === 0 ? (
            <p className="text-sm text-[#3A2D28]/70">
              No chat records returned for this user. Backend admin-level chat-history endpoint is needed for full visibility.
            </p>
          ) : (
            <div className="space-y-3">
              {chats.map((chat) => (
      ...
      {activeTab === 'bookings' && (
        <Card title="Bookings" subtitle="Reservations made by this user as guest">
          {bookings.length === 0 ? (
      ...
      {activeTab === 'earnings' && (
        <Card title="Earnings" subtitle="Revenue generated by the user's listings">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      {activeTab === 'security' && (
        <div className="grid gap-6 lg:grid-cols-2">
          <Card title="Profile Settings" subtitle="Admin can edit name and email (frontend only)">
            <div className="space-y-3">
          ...
          <Card title="Password & Danger Zone" subtitle="Change password or delete account (frontend only)">
            <div className="space-y-3">
              <label className="block text-xs font-semibold uppercase tracking-wide text-[#3A2D28]/70">
                New Password
              </label>
