    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`input-field pl-10 ${toggle ? 'pr-10' : ''} ${
        ...
        {toggle && (
          <button
            type="button"
            onClick={toggle}
            className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-primary-500
...
function RoleSelector({ role, setRole }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Je suis un(e)</span>
          ...
          <button
            ...
            className={`relative z-10 flex-1 flex items-center gap-2.5 px-3 py-2.5 rounded-xl
                        transition-colors duration-200 ${
                          role === id ? 'text-primary-700' : 'text-slate-500 hover:text-slate-700'
                        }`}
          >
            <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-colors ${
              role === id ? 'bg-primary-100' : 'bg-slate-200'
            }`}>
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Connectez-vous à votre compte Maskan</p>
      </div>
      ...
      <motion.button
        type="submit"
        whileHover={!loading && !success ? { scale: 1.02, boxShadow: '0 10px 28px rgba(99,102,241,0.35)' } : {}}
        ...
        {['Google', 'Facebook'].map((provider) => (
          <motion.button
            ...
            className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-slate-200
                       text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-300
...
function RegisterForm({ onSwitch, onClose }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div>
        <h2 className="text-2xl font-extrabold text-slate-900">Créez votre compte 🏠</h2>
        <p className="text-sm text-slate-500 mt-1">Rejoignez des milliers d'utilisateurs sur Maskan</p>
      </div>
      ...
      <label className="flex items-start gap-2 cursor-pointer group">
        ...
        />
        <span className={`text-xs leading-relaxed ${errors.agreed ? 'text-red-500' : 'text-slate-500'}`}>
          J'accepte les{' '}
    ...
    <div className="relative h-full min-h-[480px] overflow-hidden rounded-l-3xl hidden lg:block">
      ...
      <div className="absolute top-8 right-8 flex flex-col gap-2">
        {['SSL sécurisé', '18 000+ membres', 'Zéro frais cachés'].map((tag) => (
          <span
            key={tag}
            className="text-[10px] font-semibold bg-white/15 backdrop-blur-sm border border-white/25
                       text-white px-3 py-1 rounded-full text-right"
          >
            ✓ {tag}
          </span>
        ))}
      ...
      <div className="absolute bottom-10 left-8 right-8">
        <AnimatePresence mode="wait">
          ...
          >
            <p className="text-white font-medium text-sm leading-relaxed mb-3 italic">{q.text}</p>
            <footer className="text-white/60 text-xs font-semibold">— {q.author}</footer>
          </motion.blockquote>
        </AnimatePresence>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-slate-100
                     flex items-center justify-center text-slate-500 hover:bg-slate-200
                     transition-colors duration-150"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="relative overflow-hidden flex flex-col justify-center
                        px-8 py-10 sm:px-10 bg-white max-h-[95vh] overflow-y-auto">

          {/* Ambient orb */}
          <div className="absolute -top-20 -right-20 w-64 h-64 bg-primary-100 rounded-full blur-3xl pointer-events-none opacity-50" />
          <div className="absolute -bottom-20 -left-10 w-48 h-48 bg-emerald-100 rounded-full blur-3xl pointer-events-none opacity-40" />
