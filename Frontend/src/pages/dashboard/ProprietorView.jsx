...
// ── Colour map ────────────────────────────────────────────────
const CARD_COLORS = {
  indigo:  { bg: 'bg-primary-50',  icon: 'bg-primary-500', text: 'text-primary-600', bar: 'bg-primary-500' },
  blue:    { bg: 'bg-primary-100', icon: 'bg-primary-400', text: 'text-primary-500', bar: 'bg-primary-400' },
  emerald: { bg: 'bg-primary-100', icon: 'bg-primary-600', text: 'text-primary-700', bar: 'bg-primary-600' },
  amber:   { bg: 'bg-primary-200', icon: 'bg-primary-300', text: 'text-primary-700', bar: 'bg-primary-300' },
}

const STATUS_COLORS = {
  active:   'bg-primary-100 text-primary-700',
  inactive: 'bg-primary-100 text-primary-600',
    ...
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <span className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full
                          ${up ? 'bg-primary-50 text-primary-600' : 'bg-red-50 text-red-500'}`}>
          {up ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
          {Math.abs(stat.delta)}{typeof stat.delta === 'number' && stat.delta % 1 !== 0 ? '%' : ''}
        </span>
      </div>

      <p className="text-2xl font-extrabold text-primary-900 leading-none mb-1">
        <AnimatedNumber value={stat.value} />
        {stat.unit && <span className="text-sm font-semibold text-primary-400 ml-1">{stat.unit}</span>}
      </p>
      <p className="text-xs text-primary-500 font-medium">{stat.label}</p>
...
function PropertyCard({ property, index, onDelete }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden aspect-video">
        ...
        <div className="absolute top-2 right-2 relative">
          <button
            ...
            className="w-8 h-8 rounded-full bg-primary-50/80 backdrop-blur-sm flex items-center justify-center
                       text-primary-600 hover:bg-primary-100 transition-colors shadow-sm"
          >
          ...
          <AnimatePresence>
            {menuOpen && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-1 w-40 bg-primary-100 rounded-xl shadow-lg
                           border border-primary-200 py-1 z-10"
              >
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2.5 w-full px-3 py-2 text-xs text-primary-700
                                hover:bg-primary-50 transition-colors ${color}`}
                  >
        ...
        {/* Price */}
        <div className="absolute bottom-3 right-3 bg-primary-50/90 backdrop-blur-sm rounded-xl px-3 py-1.5">
          <p className="text-sm font-extrabold text-primary-900 leading-none">
            {property.price.toLocaleString('fr-TN')} TND
            <span className="text-[10px] font-normal text-primary-500">/mois</span>
          </p>
        </div>
      </div>
      ...
      {/* Body */}
      <div className="p-4">
        <h3 className="font-semibold text-primary-900 text-sm truncate">{property.title}</h3>
        <div className="flex items-center gap-1 mt-0.5 mb-3">
          <MapPin className="w-3 h-3 text-primary-400" />
          <span className="text-xs text-primary-500">{property.location}</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={label} className="bg-primary-50 rounded-xl p-2.5 text-center">
              <p className="text-sm font-bold text-primary-800">{value}</p>
              <p className="text-[10px] text-primary-400 mt-0.5">{label}</p>
            </div>
          ))}
        </div>
        ...
        {property.rating && (
          <div className="flex items-center gap-1.5 mt-3">
            ...
            <span className="text-xs font-bold text-primary-700">{property.rating}</span>
            <span className="text-[10px] text-primary-400">note moyenne</span>
          </div>
        )}
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-xl bg-primary-100 rounded-3xl shadow-2xl overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-primary-200">
          <div>
            <h2 className="text-base font-bold text-primary-900">Ajouter une propriété</h2>
          ...
          <button onClick={onClose}
                  className="w-8 h-8 rounded-full bg-primary-100 flex items-center justify-center
                             text-primary-500 hover:bg-primary-200 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          {STEPS.map((s, i) => (
            <div key={s} className="flex-1">
              <div className="flex items-center gap-2 mb-1.5">
                <motion.div
                  animate={{
                    backgroundColor: i < step ? '#B8622A' : i === step ? '#B8622A' : '#E0CEBD',
                    scale: i === step ? 1.1 : 1,
                  }}
                  className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold text-primary-50"
                >
                  {i < step ? <CheckCircle2 className="w-3.5 h-3.5" /> : i + 1}
                </motion.div>
                <span className={`text-[10px] font-semibold hidden sm:block ${i === step ? 'text-primary-600' : i < step ? 'text-primary-500' : 'text-primary-300'}`}>
                  {s}
                </span>
              </div>
              <motion.div
                animate={{ backgroundColor: i <= step ? '#B8622A' : '#E0CEBD' }}
          ...
          <AnimatePresence mode="wait" custom={direction}>
              ...
              <motion.div key="step0" custom={direction} variants={stepVar}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="block text-xs font-bold text-primary-500 uppercase tracking-wider mb-1.5">
                      Titre de l'annonce *
                    </label>
                  ...
                  <div>
                    ...
                    <input value={form.city} onChange={e => setForm(f => ({ ...f, city: e.target.value }))}
                           placeholder="Tunis, Hammamet…" className={inputClass} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-primary-500 uppercase tracking-wider mb-1.5">
                      Chambres
            ...
            {step === 1 && (
              <motion.div key="step1" custom={direction} variants={stepVar}
                          initial="enter" animate="center" exit="exit"
                          className="space-y-4">
                <div className="border-2 border-dashed border-primary-200 rounded-2xl p-8
                                text-center hover:border-primary-300 transition-colors cursor-pointer group">
                  <Upload className="w-8 h-8 text-primary-300 group-hover:text-primary-400 mx-auto mb-3 transition-colors" />
                  <p className="text-sm font-semibold text-primary-600">Glissez vos photos ici</p>
                  <p className="text-xs text-primary-400 mt-1">PNG, JPG, WEBP — max 10 Mo par fichier</p>
                  <button className="mt-4 px-5 py-2 rounded-xl bg-primary-50 text-primary-700 text-xs font-bold
                                     hover:bg-primary-100 transition-colors">
                    Parcourir les fichiers
                  </button>
                </div>
                <p className="text-xs text-primary-400 text-center">
                  La <span className="font-semibold text-primary-600">première photo</span> sera l'image principale.
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" value={form.price}
                           ...
                           placeholder="1800" className={inputClass} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-primary-500 uppercase tracking-wider mb-1.5">
                      Dépôt de garantie
                    ...
                    <input type="number" value={form.deposit}
                           ...
                           placeholder="3600" className={inputClass} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-primary-500 uppercase tracking-wider mb-1.5">
                      Séjour min. (jours)
                      ...
                      return (
                        <motion.button
                          ...
                          className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all duration-150
                                      ${sel
                                        ? 'bg-primary-500 text-primary-50 border-primary-500 shadow-sm'
                                        : 'bg-primary-100 text-primary-600 border-primary-200 hover:border-primary-300'}`}
                        >
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-6 py-4 border-t border-primary-200">
          <button onClick={prev} disabled={step === 0}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold
                             text-primary-600 hover:bg-primary-100 disabled:opacity-30 transition-colors">
          ...
          {step < STEPS.length - 1 ? (
            <motion.button
              ...
              className="flex items-center gap-1.5 px-6 py-2.5 rounded-xl text-sm font-bold
                         text-primary-50 bg-primary-600 hover:bg-primary-700 shadow-sm transition-colors"
            >
            ...
            <motion.button
              ...
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold text-primary-50
                          shadow-sm transition-all duration-200
                          ${done ? 'bg-primary-500' : 'bg-primary-600 hover:bg-primary-700'}`}
            >
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-lg font-extrabold text-primary-900">Mes Propriétés</h2>
            <p className="text-xs text-primary-400 mt-0.5">{properties.length} annonces au total</p>
          </div>
          <motion.button
            whileHover={{ scale: 1.04, boxShadow: '0 8px 24px rgba(164,131,116,0.30)' }}
            whileTap={{ scale: 0.97 }}
            onClick={() => setShowModal(true)}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary-600 text-primary-50
                       text-sm font-bold shadow-md hover:bg-primary-700 transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.02, borderColor: '#B8622A' }}
            onClick={() => setShowModal(true)}
            className="flex flex-col items-center justify-center gap-3 rounded-2xl
                       border-2 border-dashed border-primary-200 min-h-[240px]
                       text-primary-400 hover:text-primary-600 transition-all duration-200"
          >
      ...
      <div>
        <h2 className="text-lg font-extrabold text-primary-900 mb-4">Réservations récentes</h2>
        <div className="bg-primary-100 rounded-2xl border border-primary-200 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-primary-200">
                {['Locataire', 'Propriété', 'Dates', 'Montant', 'Statut'].map(h => (
                  <th key={h} className="text-left px-4 py-3 text-xs font-bold uppercase tracking-wider text-primary-400">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {RECENT_BOOKINGS_PROP.map((b, i) => (
                <motion.tr
                  ...
                  className="border-b border-primary-200 last:border-none hover:bg-primary-50 transition-colors"
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <img src={b.avatar} alt={b.tenant}
                           className="w-7 h-7 rounded-full object-cover ring-1 ring-primary-200" />
                      <span className="font-medium text-primary-800 text-xs">{b.tenant}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-primary-600 max-w-[160px] truncate">{b.property}</td>
                  <td className="px-4 py-3 text-xs text-primary-500">{b.dates}</td>
                  <td className="px-4 py-3 text-xs font-bold text-primary-800">
