...
function CountdownCell({ value, label }) {
  return (
    <div className="flex flex-col items-center bg-white/20 rounded-xl px-2.5 py-1.5">
      <motion.span
        key={value}
      ...
      <span className="text-[9px] text-emerald-100 uppercase tracking-wider mt-0.5">{label}</span>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="relative p-5">
        {isFirst && (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-emerald-400/30
                           text-emerald-100 px-2.5 py-1 rounded-full mb-3">
            <TrendingUp className="w-3 h-3" /> Prochain séjour
          </span>
        )}

        <h3 className="font-bold text-white text-sm mb-0.5 truncate">{stay.property}</h3>
        <div className="flex items-center gap-1 mb-4">
          <MapPin className="w-3 h-3 text-emerald-300" />
          <span className="text-xs text-emerald-200">{stay.location}</span>
        </div>
        ...
        {/* Countdown */}
        <div>
          <p className="text-[10px] text-white/50 uppercase tracking-widest mb-2">Compte à rebours</p>
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <button className="flex items-center gap-1 text-[11px] font-bold text-emerald-300
...
function SavedCard({ home, index }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4, boxShadow: '0 16px 40px rgba(16,185,129,0.10)' }}
    ...
    >
      <div className="relative aspect-video overflow-hidden">
        ...
        {/* Price tag */}
        <div className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-sm rounded-xl px-2.5 py-1.5">
          <p className="text-sm font-extrabold text-slate-900 leading-none">
            {home.price.toLocaleString('fr-TN')}
            <span className="text-[10px] font-normal text-slate-500"> TND/mois</span>
          </p>
        </div>
      </div>

      <div className="p-4">
        <h3 className="font-semibold text-slate-900 text-sm truncate">{home.title}</h3>
        <div className="flex items-center gap-1 mt-0.5 mb-3">
          <MapPin className="w-3 h-3 text-slate-400" />
          <span className="text-xs text-slate-500">{home.location}</span>
        </div>
        ...
        <motion.button
          whileHover={{ scale: 1.03, boxShadow: '0 6px 20px rgba(16,185,129,0.25)' }}
          whileTap={{ scale: 0.97 }}
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-extrabold text-slate-900">Séjours à venir</h2>
            <p className="text-xs text-slate-400 mt-0.5">{UPCOMING_STAYS.length} réservation{UPCOMING_STAYS.length > 1 ? 's' : ''} confirmée{UPCOMING_STAYS.length > 1 ? 's' : ''}</p>
          </div>
          <div className="flex items-center gap-2">
            <button
        ...
        {UPCOMING_STAYS.length === 0 ? (
          <div className="flex flex-col items-center justify-center bg-white rounded-2xl border border-slate-100
                          border-dashed py-14 text-center">
            <CalendarDays className="w-10 h-10 text-slate-200 mb-3" strokeWidth={1.5} />
            <p className="text-sm font-semibold text-slate-500">Aucun séjour à venir</p>
            <p className="text-xs text-slate-400 mt-1">Explorez et réservez votre prochain logement.</p>
            <button className="mt-4 px-5 py-2.5 rounded-xl bg-emerald-500 text-white text-xs font-bold
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-extrabold text-slate-900">Logements sauvegardés</h2>
            <p className="text-xs text-slate-400 mt-0.5">{SAVED_HOMES.length} favoris</p>
          </div>
      ...
      {/* ── Search History ────────────────────────────────── */}
      <div>
        <h2 className="text-lg font-extrabold text-slate-900 mb-4">Historique de recherche</h2>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5">
                  ...
                  {item.price && (
                    <span className="text-[11px] text-slate-400">· max {item.price.toLocaleString('fr-TN')} TND</span>
                  )}
                  <span className="flex items-center gap-1 text-[11px] text-slate-400">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <motion.button
                  whileHover={{ x: 2 }}
                  className="opacity-0 group-hover:opacity-100 text-emerald-500 transition-all"
                >
