...
const ROLE_COLORS = {
  ...
  PROPRIETAIRE: 'bg-primary-100 text-primary-600',
  TENANT:       'bg-primary-50 text-primary-500',
}
const STATUS_DOT = {
...
function StatCard({ stat, index }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-2xl font-extrabold text-primary-900 leading-none">
        {typeof stat.value === 'number' ? stat.value.toLocaleString('fr-FR') : stat.value}
      </p>
      <p className="text-xs text-primary-500 font-medium mt-1">{stat.label}</p>
...
// ── SVG Line Chart ────────────────────────────────────────────
function LineChart({ data, labels, color = '#B8622A', label }) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex justify-between px-3 mt-1">
        {labels.map(l => (
          <span key={l} className="text-[10px] text-primary-400">{l}</span>
        ))}
      </div>
    </div>
  )
...
function UserRow({ user, index, onBan, onPromote }) {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <img src={user.avatar} alt={user.name}
                 className="w-8 h-8 rounded-full object-cover ring-1 ring-primary-200" />
            <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-primary-100
                              ${STATUS_DOT[user.status] || 'bg-primary-300'}`} />
          </div>
          <div>
      ...
      <td className="px-4 py-3.5">
        <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${ROLE_COLORS[user.role] || 'bg-primary-100 text-primary-600'}`}>
          {user.role}
        </span>
      </td>
      <td className="px-4 py-3.5 text-xs text-primary-500">{user.joined}</td>
      <td className="px-4 py-3.5 text-xs font-medium text-primary-700">{user.properties ?? '-'}</td>
      <td className="px-4 py-3.5">
        <div className="relative flex items-center gap-2">
          {loading && <Loader2 className="w-3.5 h-3.5 text-primary-400 animate-spin" />}
          <button onClick={() => setMenuOpen(v => !v)}
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-primary-400
          ...
          <AnimatePresence>
            {menuOpen && (
              <motion.div
                ...
                className="absolute right-0 bottom-full mb-1 w-44 bg-primary-100 rounded-xl shadow-lg
                           border border-primary-200 py-1 z-20"
              >
                {[
                  ...
                  <button key={label}
                          ...
                          className={`flex items-center gap-2.5 w-full px-3 py-2 text-xs
                                      ...
                                      ${danger ? 'text-red-500 hover:bg-red-50' : 'text-primary-700'}`}>
    ...
    >
      <div className="relative h-36 overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between">
          <div>
            <p className="text-sm font-bold text-primary-50 truncate">{property.title}</p>
            <div className="flex items-center gap-1 mt-0.5">
              <MapPin className="w-3 h-3 text-primary-200" />
              <span className="text-xs text-primary-200">{property.location}</span>
            </div>
          </div>
          <div className="bg-primary-50/90 backdrop-blur-sm rounded-xl px-2.5 py-1.5">
      ...
      <div className="p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div className="min-w-0">
            <p className="text-xs font-semibold text-primary-700 truncate">{property.owner}</p>
            <p className="text-[10px] text-primary-400">{property.submittedAt}</p>
          </div>
          <div className="ml-auto flex items-center gap-1">
            <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
            <span className="text-xs font-bold text-primary-700">{property.ownerRating}</span>
          </div>
        </div>
            ...
            <>
              <motion.button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl
                           bg-primary-500 text-primary-50 text-xs font-bold hover:bg-primary-600
...
export default function AdminView() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Growth Chart ──────────────────────────────────── */}
      <div className="bg-primary-100 rounded-2xl border border-primary-200 shadow-sm p-5">
        <h2 className="text-base font-bold text-primary-900 mb-1">Croissance de la plateforme</h2>
        <p className="text-xs text-primary-400 mb-5">Utilisateurs et annonces sur les 7 derniers mois</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <p className="text-xs font-bold text-primary-500 uppercase tracking-wider mb-2">Utilisateurs</p>
            <LineChart
          ...
          <div>
            <p className="text-xs font-bold text-primary-500 uppercase tracking-wider mb-2">Propriétés</p>
            <LineChart
      ...
      <div>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div>
            <h2 className="text-lg font-extrabold text-primary-900">Gestion des utilisateurs</h2>
            <p className="text-xs text-primary-400 mt-0.5">{users.length} utilisateurs inscrits</p>
          </div>
          {/* Filter pills */}
              ...
              {ROLE_FILTERS.map(r => (
                <motion.button
                  ...
                  className={`relative px-3 py-1.5 rounded-full text-xs font-bold transition-colors
                              ${filter === r ? 'text-primary-50' : 'text-primary-600 hover:bg-primary-100'}`}
                >
                  {filter === r && (
                    <motion.span
        ...
        <div className="bg-primary-100 rounded-2xl border border-primary-200 shadow-sm overflow-hidden">
          ...
          {/* Pagination */}
          {pageCount > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-primary-200">
              <span className="text-xs text-primary-400">
                {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, roleFilter.length)} / {roleFilter.length}
              </span>
              <div className="flex items-center gap-1">
                <button onClick={() => setPage(p => Math.max(p - 1, 0))} disabled={page === 0}
                        className="w-7 h-7 rounded-lg flex items-center justify-center text-primary-500
                                   hover:bg-primary-100 disabled:opacity-30 transition-colors">
                  <ChevronLeft className="w-4 h-4" />
                </button>
                {Array.from({ length: pageCount }, (_, i) => (
                  <button key={i} onClick={() => setPage(i)}
                          className={`w-7 h-7 rounded-lg text-xs font-bold transition-colors
                                      ${i === page ? 'bg-primary-700 text-primary-50' : 'hover:bg-primary-100 text-primary-600'}`}>
                    {i + 1}
                  </button>
                ))}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-extrabold text-primary-900">File d'approbation</h2>
        ...
        {pending.length === 0 ? (
          <motion.div
            ...
            className="flex flex-col items-center justify-center bg-primary-100 rounded-2xl border border-dashed
                       border-primary-200 py-14 text-center"
          >
            <CheckCircle2 className="w-10 h-10 text-primary-400 mb-3" strokeWidth={1.5} />
            <p className="text-sm font-semibold text-primary-600">File vide !</p>
            <p className="text-xs text-primary-400 mt-1">Toutes les annonces ont été traitées.</p>
          </motion.div>
        ) : (
